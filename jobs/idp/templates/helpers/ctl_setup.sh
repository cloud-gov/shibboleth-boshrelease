...
do
  ...
  for package_bin_dir in $(find ${package_dir} -type d -name *bin)
  do
  ...
  then
    export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:${package_dir}/lib
  fi
done
...
for java_jar in $(find /var/vcap/packages -type f -name *.jar)
do
